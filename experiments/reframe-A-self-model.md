...
- **It dissolves the method's biggest standing risk.** The curated catalogue / layering-spec rots (R4) and its freshness is the steward's hardest duty (R10). A *derived* self-model is never stale — regenerated from code on demand — so the staleness risk that the whole governance frame leaned on a human to manage largely goes away. This is the single most consequential reframe in the chain.
...
## Honest caveats
- **The fixture still had a visible chokepoint** (`create` materialises both shapes), so A0 *could* have succeeded via it — 1/8 did. A0's failures were "didn't look beyond billing," which the self-model fixed by naming all sites. A cleaner no-chokepoint fixture would push A0 toward 0/8 and is the honest follow-up.
...
→ book: a major beat in Ch.3/Ch.6/Ch.7 and Ch.11 — the central artefact is a *derived self-model*, not a maintained document; it is the visibility mechanism (surfaces the divergent sites) and it is never stale (regenerated from code), which is the honest answer to the R4 staleness risk and the R10 freshness duty. Engineering gaps named: a live query tool, a real extractor, retrieval quality at scale, and the no-chokepoint confirmation.
