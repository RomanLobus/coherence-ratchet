...
`scripts/probe_hyperliminal.py`, `git log --all` history (robust to the src/ migration), Jaccard ≥ 0.25, co ≥ 3:
...
## Artefacts
- `scripts/probe_hyperliminal.py` — reuses `archmetrics` (static graph) + git history; `--all` + package-relative
