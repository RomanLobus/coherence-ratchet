...
- **Together, A + B resolve the catalogue-staleness problem (R4) and the steward's freshness duty (R10).** A makes the artefact *derived* (never stale, surfaces every site); B makes the canonical entries *emergent-ratified* (sound for common concepts) rather than hand-written. The steward's role shrinks from *author + refresh* (the rot-prone duty) to *ratify + judge* (cheap, durable). This is the strongest synthesis the reframe chain produced.
