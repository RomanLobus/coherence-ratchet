...
(`probe_priority.py`) multiplies them into one ranking and tests whether the product prioritises the
...
## Artefacts
- `probe_priority.py` — reuses `probe_cochange` (co-change) and `probe_connascence` (locality) internals.
