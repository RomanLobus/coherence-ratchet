...
Answers the questions an agent (or a steward) asks before making a change:
  ...
  - "what <entity> shapes were observed?"   -> entity
...
def _concept(model, subj):
    ...
    hits = []
    for f in observed.get("functions", []):
    ...
    candidates = [
        c for c in model.get("candidates", [])
        if c.get("kind") == "reuse_helper"
        and any(s in _name_tokens(c.get("suggestion", {}).get("concept", "")) for s in subj)
    ]
    return {"matches": hits, "candidates": candidates}
...
def _entity(model, subj):
    ...
    for e in observed.get("entities", []):
        ...
        if e["kind"] == "dict-shape":
            n = len(e["sites"])
            common = sorted(k for k, c in e["key_frequency"].items() if c == n)
            out.append({"name": e["name"], "kind": e["kind"],
                        "keys_observed_at_every_site": common,
                        "sites": e["sites"], "per_site_keys": e["per_site_keys"],
                        "truth": "OBSERVED"})
        else:
...
def _helper(model, subj):
    ...
    for candidate in model.get("candidates", []):
        if candidate.get("kind") != "reuse_helper":
            continue
        concept = candidate.get("suggestion", {}).get("concept", "")
        ctoks = _name_tokens(concept)
        if any(s in ctoks or s == concept for s in subj):
            hits.append(candidate)
    fns = [f["qualname"] for f in observed.get("functions", [])
...
def _convention(model, subj):
    observed = model.get("observed", model)
    out = []
    for c in observed.get("conventions", []):
...
def answer(model: dict, question: str, intent: dict | None = None, use_llm: bool = False) -> dict:
    query_intent = _intent(question)
    subj = _subject_tokens(question)
    result = _DISPATCH[query_intent](model, subj)
    # Superseded records stay in the intent file so intent keeps a history, and they must not be
    # rendered as live [RATIFIED] lines here any more than in the grounding pack: both surfaces
    # print the same imperative label, and a decision that has been replaced is no longer one.
    # Filtering the pack alone left this renderer answering with the superseded revision beside
    # the current one, indistinguishable.
    ratified = [item for item in (intent or {}).get("ratifications", [])
                if not item.get("superseded_by")]
    result.update({"intent": query_intent, "subject": subj, "question": question, "matcher": "deterministic",
                   "truth": "OBSERVED", "ratified": ratified})
    if use_llm:
        try:
            ranked = _llm_rerank(model, question, query_intent, result)
...
def _llm_rerank(model, question, intent, det_result):
    ...
    observed = model.get("observed", model)
    candidates = [f["qualname"] for f in observed.get("functions", [])]
    prompt = (
...
def render(result: dict) -> None:
    ...
    if not matches:
        ...
        for candidate in result.get("candidates", []):
            ...
            print(f"  [CANDIDATE] {candidate['id']}: possible reuse site "
                  f"{suggestion.get('reuse_site')} for {suggestion.get('concept')}")
    elif intent == "entity":
        for e in matches:
            if e["kind"] == "dict-shape":
                print(f"  [OBSERVED] {e['name']} (dict-shape) keys seen at every site: "
                      f"{', '.join(e['keys_observed_at_every_site']) or '(none shared)'}")
                print(f"    seen in: {', '.join(e['sites'])}")
                for module, keys in e.get("per_site_keys", {}).items():
        ...
        for h in matches:
            ...
            print(f"  [CANDIDATE] {h['id']}: possible reuse of {suggestion.get('reuse_site')}")
        ...
        for c in matches[:15]:
            print(f"  [OBSERVED] {c['value']}  in {', '.join(c['modules'])}")
    if result.get("ratified"):
        print("\n  Ratified intent:")
        for item in result["ratified"]:
            print(f"  [RATIFIED] {item.get('contract')} within {item.get('scope', '.')}")
    if result.get("llm"):
