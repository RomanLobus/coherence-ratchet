...
class ArchSnapshot:
    n_modules: int
    n_edges: int
    unreadable_modules: int         # files the parser could not read; their edges are missing
...
def _edges_for(path: str, mod: str, pkg: str, internal: set[str],
               unreadable: list[str] | None = None) -> set[str]:
    """Internal import edges out of one module.

    A file this parser cannot read contributes no edges, which is indistinguishable from a module
    that genuinely imports nothing. That is a failure reading as a clean result, and it moved a real
    number: two Python-2-era files in an early flask tree parse under one interpreter and not
    another, so the same pinned commit measured 9 edges or 18 depending on the Python running the
    analyser. The caller is now told, and the count travels with the snapshot.
    """
    try:
...
def measure_arch(root: str) -> ArchSnapshot:
    ...
    for mod, path in mods.items():
        adj[mod] |= _edges_for(path, mod, pkg, internal, unreadable)
    n = len(internal)
    ...
    return ArchSnapshot(
        n_modules=n,
        n_edges=n_edges,
        unreadable_modules=len(unreadable),
