...
def test_report_groups_open_items_by_exposure_and_ignores_closed():
    path = _ledger([
        {"when": "2026-06-01", "region": "a", "owner": "t", "repayment_trigger": "x",
         "exposure_tier": "HIGH", "status": "accepted"},
        {"when": "2026-06-02", "region": "b", "owner": "t", "repayment_trigger": "x",
         "exposure_tier": "LOW", "status": "closed"},
    ])
    result = report.report(path, today=TODAY)
    assert result["total"] == 1
    assert result["by_exposure"] == {"HIGH": 1}
